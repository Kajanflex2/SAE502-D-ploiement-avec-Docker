...
function sendmail_verify($username,$email,$verification_token) {
    ...
    $email_format = "
        ...
        <div class='email-container'>
            ...
            <h5>Confirmez votre e-mail en cliquant sur le lien ci-dessous :</h5>
            <a href='http://localhost:8081/verify.php?token=$verification_token' target='_blank' class='confirmation-link'>Cliquez ici</a>
        </div>
        </body>
        </html>";
   ...
   // validation les données reçu
    if(!$firstname){
        ...
        $_SESSION['signup']="Le mot de passe doit comporter 8 caractères";
            ...
            $_SESSION['signup']="Le mot de passe doit contenir au moins un chiffre, une majuscule, une minuscule et un caractère spécial."; 
    }elseif(!$avatar['name']){
...
if(isset($_SESSION['signup'])){
    ...
    if(!mysqli_errno($conn)){
        ...
        //function sendmail_verify pour envoyer un mail
        sendmail_verify("$username","$email","$verification_token");
